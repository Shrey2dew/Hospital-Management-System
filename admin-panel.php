  ...
  if(date("Y-m-d",$appdate1)>=$cur_date){
    if((date("Y-m-d",$appdate1)==$cur_date and date("H:i:s",$apptime1)>$cur_time) or date("Y-m-d",$appdate1)>$cur_date) {
      ...
      if(mysqli_num_rows($check_query) == 0) {
        $stmt = $con->prepare("INSERT INTO appointmenttb (fname, lname, gender, email, contact, doctor, docFees, appdate, apptime, userStatus, doctorStatus) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    
        // Bind parameters
        $stmt->bind_param("ssssssisiii", $fname, $lname, $gender, $email, $contact, $doctor, $docFees, $appdate, $apptime, $userStatus, $doctorStatus);
        ...
        $appdate = $appdate;
        $apptime = $apptime;
        $userStatus = 1; // Assuming these are integer values
        $doctorStatus = 1;
    
        // Execute statement
        if($stmt->execute()) {
            $last_id = $con->insert_id; // Get the last inserted id
            echo "<script>alert('Your appointment successfully booked with ID: $last_id');</script>";
        } else {
